...
function setupResizerEvents() {
	document.body.addEventListener("mousedown", function (md) {
		...
		if (target.nodeType !== 1 || !target.classList.contains("flex-container__flex-resizer")){ 
      /* target.tagName !== "FLEX-RESIZER") {*/
			return;
		}
		var parent = target.parentNode;
		var h = parent.classList.contains("flex-container_horizontal");
		var v = parent.classList.contains("flex-container_vertical");
		if (h && v) {
			return;
		} else if (h) {
